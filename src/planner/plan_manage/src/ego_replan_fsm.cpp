...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    {
      string exploration_goal_topic;
      if (planner_manager_->pp_.drone_id <= -1)
      {
        exploration_goal_topic = string("/drone_single_planning/exploration_goal");
      }
      else
      {
        exploration_goal_topic = string("/drone_") + std::to_string(planner_manager_->pp_.drone_id) + string("_planning/exploration_goal");
      }

      exploration_goal_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
          exploration_goal_topic,
          1,
          [this](const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
          {
            this->explorationGoalCallback(msg);
          });

      RCLCPP_INFO(node_->get_logger(), "Waiting for exploration goal from %s", exploration_goal_topic.c_str());
    }
    else
  ...
  void EGOReplanFSM::explorationGoalCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    if (msg->pose.position.z < -0.1)
      return;
    ...
    if (!have_odom_)
    {
      RCLCPP_WARN(node_->get_logger(), "Received exploration goal before odometry, ignore.");
      return;
    }

    cout << "Exploration target received!" << endl;
